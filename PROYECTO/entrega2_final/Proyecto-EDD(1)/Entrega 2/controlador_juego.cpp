...
std::list<std::string> controlador_juego::palabras_prefijo;
std::list<std::string> controlador_juego::palabras_sufijo;
puntaje controlador_juego::puntajeObject;
...
bool controlador_juego::vComando(const std::vector<std::string> &strings) {
  ...
  if (arg1 == "inicializar") {
    ...
    if (!arg2.empty()) {
      ...
      std::cout << "Modo de uso: palabras_prefijo [prefijo]\n";
      std::cout << "Por favor intente de nuevo\n";
    }
    return true;
    ...
    if (!arg2.empty()) {
      ...
      std::cout << "Modo de uso: palabras_sufijo [sufijo]\n";
      std::cout << "Por favor intente de nuevo\n";
    }
    return true;
    ...
    if (!diccionario.empty())
      diccionario.clear();
    if (!diccionario_inverso.empty())
      diccionario_inverso.clear();
    if (!arbol.esVacio())
      arbol.limpiar();
    if (!arbol_inverso.esVacio())
      arbol_inverso.limpiar();
    ...
    std::cout << "No es un comando válido\n";
  }
  ...
  void controlador_juego::inicializarDiccionarioInverso(std::string fileName) {
    ...
    // for (std::list<std::string>::const_iterator tIt =
    // this->diccionario_inverso.begin(); tIt !=
    // this->diccionario_inverso.end(); tIt++)
    //   {
    //     std::cout << *tIt << std::endl;
    //   }
  }
  ...
  void controlador_juego::inicializarArbol(std::string fileName) {
    ...
    for (it = lines.begin(); it != lines.end(); it++) {
      if (std::regex_match(*it, inValido)) {
        // Printear las palabras antes de insertar al arbol:::::
        // std::cout << *it << std::endl;
        inicializarArbol(*it, arbol.obtenerRaiz());
      }
    }
    std::cout << "El árbol ha sido inicializado correctamente." << std::endl;
  ...
  void controlador_juego::inicializarArbolInverso(std::string fileName) {
    ...
    while (std::getline(inFile, line)) {
      ...
      if (inverseLine.length() > 1)
        lines.push_back(inverseLine);
    }

    // Dejamos la lista inversa en orden alfabetico para poder reducir el tamano
    // del arbol
    lines.sort();
    ...
    for (it = lines.begin(); it != lines.end(); it++) {
      if (std::regex_match(*it, inValido)) {
        inicializarArbol(*it, arbol_inverso.obtenerRaiz());
      }
    }
  ...
  // NO SE USA; ES LO MISMO QUE INICIALIZARARBOL
  ...
  void controlador_juego::palabrasPorPrefijo(std::string prefijo){
    puntajeObject = puntaje();
    
    std::regex inValido("[A-Za-z]+");
    if (!std::regex_match(prefijo, inValido)) {
      std::cout << "Sufijo invalido, intente de nuevo." << std::endl;
      return;
    }
    
    if (this->arbol.esVacio()) {
      std::cout << "No hay árboles cargados, no se puede ejecutar la operación"
                << std::endl;
      std::cout << "Por favor inicializar algun diccionario antes de continuar."
                << std::endl;
      return;
    ...
    if (prefijo.size() < 2) {
      std::cout
          << "El prefijo debe tener al menos dos letras."
          << std::endl;
    ...
    for (size_t i = 0; i < prefijo.size(); ++i) {
      ...
      for (std::list<NodoGeneral<char> *>::iterator it =
               nodoPrefijo->desc.begin();
           it != nodoPrefijo->desc.end(); ++it) {
        if ((*it)->obtenerDato() == c) {
          nodoPrefijo = *it;
          encontrado = true;
          break;
      ...
      if (!encontrado) {
        std::cout << "No se encuentra el prefijo en el diccionario" << std::endl;
        return;
      }
    }

    std::cout << "Las palabras que inician con el prefijo '"
              << prefijo << "' son:" << std::endl;
    std::vector<std::string> palabras;
    encontrarPalabras(nodoPrefijo, prefijo, palabras);
    for (std::vector<std::string>::iterator it = palabras.begin();
         it != palabras.end(); ++it) {
      std::cout << *it << " (Longitud: " << it->size()
                << ", El puntaje es:" << puntajeObject.calcularPuntaje(*it)
                << ")" << std::endl;
    }
  }
  ...
  void controlador_juego::palabrasPorSufijo(std::string sufijo) {
    ...
    std::regex inValido("[A-Za-z]+");
    if (!std::regex_match(sufijo, inValido)) {
      std::cout << "Sufijo invalido, intente de nuevo." << std::endl;
      return;
    }
    
    if (this->arbol_inverso.esVacio()) {
      std::cout << "No hay árboles cargados, no se puede ejecutar la operación"
                << std::endl;
      std::cout << "Por favor inicializar algun diccionario antes de continuar."
                << std::endl;
    ...
    if (sufijo.size() < 2) {
      std::cout << "El sufijo debe tener al menos dos letras."
                << std::endl;
      std::cout<< "Por favor intente nuevamente." << std::endl;
      return;
    }

    sufijo = invertString(sufijo);
    ...
    for (size_t i = 0; i < sufijo.size(); ++i) {
      ...
      bool encontrado = false;
      for (std::list<NodoGeneral<char> *>::iterator it =
               nodoSufijo->desc.begin();
           it != nodoSufijo->desc.end(); ++it) {
        if ((*it)->obtenerDato() == c) {
          nodoSufijo = *it;
          encontrado = true;
          break;
      ...
      if (!encontrado) {
        std::cout << "No se encuentra el sufijo en el diccionario" << std::endl;
        return;
      }
    }

    std::cout << "Las palabras que terminan con el sufijo '"
              << sufijo << "' son:" << std::endl;
    std::vector<std::string> palabras;
    encontrarPalabras(nodoSufijo, sufijo, palabras);
    if (palabras.empty()) {
      std::cout
          << "No hay palabras que terminen con este sufijo en el diccionario."
          << std::endl;
    } else {
      for (std::vector<std::string>::iterator it = palabras.begin();
           it != palabras.end(); ++it) {
        std::cout << invertString(*it) << " (Longitud: " << it->size()
                  << ", El puntaje es:" << puntajeObject.calcularPuntaje(*it)
                  << ")" << std::endl;
      }
    }
  ...
  void controlador_juego::encontrarPalabras(
      NodoGeneral<char> * nodo, std::string & palabra,
      std::vector<std::string> & palabrasEncontradas) {
    // Si el nodo actual es una hoja, agregamos la palabra completa encontrada a
    ...
    for (std::list<NodoGeneral<char> *>::iterator it = nodo->desc.begin();
         it != nodo->desc.end(); ++it) {
      char c = (*it)->obtenerDato();
      palabra.push_back(
          c); // Añadimos el carácter del nodo actual a la palabra.
      encontrarPalabras(*it, palabra, palabrasEncontradas);
      palabra.pop_back(); // Deshacemos la adición del carácter para explorar
                          // otras ramas.
    }
  }
